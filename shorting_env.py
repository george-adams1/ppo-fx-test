...
class ForexTradingEnv(gym.Env):
    def __init__(self, df, train, order_size, initial_balance, state_features, reward_history, reward_type='sortino',
        ...
        self.shorted_shares_value = {i: 0 for i in range(self.num_currencies)}
        self.n_shares_shorted = {i: 0 for i in range(self.num_currencies)}
    ...
    def step(self, action):
        ...
        for i in range(self.num_currencies):
            if action[i] == 1:  # Buy
                # normal buy
                if self.account_balance >= self.order_size and self.portfolio[i] == 0 and self.n_shares_shorted[
                    i] == 0 and self.candle_data[i][self.name_order_dic["Close"]] != 0:
                ...
                # buy to cover
                elif self.n_shares_shorted[i] > 0 and self.candle_data[i][self.name_order_dic["Close"]] != 0:
                    # buy to cover logic
                    shorted_order_value = self.shorted_shares_value[i] * self.n_shares_shorted[i]
                    current_order_value = self.candle_data[i][self.name_order_dic["Close"]] * self.n_shares_shorted[i]
                    self.account_balance += (current_order_value - shorted_order_value)
                    self.n_shares_shorted[i] = 0
                    self.shorted_shares_value[i] = 0
                ...
                elif self.account_balance > self.order_size and self.portfolio[i] == 0 and self.n_shares_shorted[
                    i] == 0 and self.candle_data[i][self.name_order_dic["Close"]] != 0:
                    # shorting logic
                    # self.account_balance -= self.order_size
                    pcs_short_sold = self.order_size / self.candle_data[i][self.name_order_dic["Close"]]
                    self.n_shares_shorted[i] = pcs_short_sold
                    self.shorted_shares_value[i] = self.candle_data[i][
                        self.name_order_dic["Close"]]  # Store the buying price
        ...
        # calculating unrealized gains in shorted shares
        shorted_value = 0
        for i in range(self.num_currencies):
            shorted_order_value = self.shorted_shares_value[i] * self.n_shares_shorted[i]
            current_order_value = self.candle_data[i][self.name_order_dic["Close"]] * self.n_shares_shorted[i]
            shorted_value += (shorted_order_value - current_order_value)

        self.total_asset = self.account_balance + sum(
            [self.candle_data[i][self.name_order_dic["Close"]] * owned for i, owned in
             enumerate(self.portfolio)]) + shorted_value
